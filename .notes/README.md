...
- **离线 zip**：`npm run bundle` → `release/sdd-cli-offline-*.zip`（内含 `.tgz` + `INSTALL.md` + **`delivery/`** + 完整源码 zip）；`release/` 已 gitignore。
- **交付文件夹**：根目录 [`delivery/`](../delivery/README.md)；与 `docs/` 同步命令 `npm run delivery:sync`。
