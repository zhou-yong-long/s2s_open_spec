...
**给 AI 的一键交付包（安装 Prompt + IDE Prompt + PM/Dev/QA 全链路用例 + 补充清单）**：

- 源码树内：**[docs/ai-delivery/README.md](./ai-delivery/README.md)**（维护入口）。
- **对外整包副本**：根目录 **[`delivery/`](../delivery/README.md)**（执行 `npm run delivery:sync` 与 `docs/` 同步；离线 zip 内亦含 `delivery/`）。

其中安装 Prompt **要求使用分支 `feature/sdd-karmastudio-pack`**，勿默认使用 `main`。
